...
- `pkg/profile/` — The backup-profiles YAML format: `Config`, `Profile`, `Store`, `Auth`, `Load`, `Save`, `LoadOrEmpty`, `EnsureMaps`, `Normalize`. Public because the profiles file is user-facing and worth scripting against without opening a repository.
...
- `internal/pathmatch/` — Glob matching for slash-separated paths: `path.Match` per segment plus `**` for "zero or more segments", which `path.Match` alone cannot express. Used by `find`'s `-name`/`-path` patterns and intended as the shared home for backup exclude patterns.
