...
4. **CLI Integration** — For new commands:
   ...
   - Declare the command in its own `cmd_<name>.go` with `leaf(name, summary, run, opts...)`, or `group(name, summary, children...)` for a command with subcommands (`command.go`). Groups need no dispatch code: `command.execute` handles subcommand lookup, the subcommand usage listing, and unknown-subcommand errors for every group.
   ...
   - Do **not** hand-edit `completion.go` for a leaf command's flags: bash, zsh, and fish per-command entries are generated from its `flagSpec`s, including value completers (`withCompleter`) and repeatable markers (`asRepeatable`). Use `withShortUsage` when the full usage text is too long for a completion menu. Any completer you name must be defined in the zsh script and mapped in `fishValueSpec` — `TestDeclaredCompletersExistInZsh` and `TestDeclaredCompletersHaveFishMapping` fail otherwise. Grouped subcommands (`auth`/`store`/`profile`/`key`) still keep hand-written completion blocks.
