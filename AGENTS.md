...
| `CLOUDSTIC_OBJECT_CACHE_DIR` | Directory for the local object cache (`DiskCacheStore`). Empty, the default, disables it. |
| `CLOUDSTIC_OBJECT_CACHE_BYTES` | Bytes that directory may hold; defaults to `storelayer.DiskCacheBudget` (2 GiB). Malformed or non-positive falls back to the default — no value of it removes the bound. |
