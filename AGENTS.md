...
4. **CLI Integration** — For new commands:
   ...
   - Keep semantic validation and derivation that depends on multiple values at the start of `run<Name>` (or in a focused helper called from it). Input binding belongs to the declaration; domain rules do not belong to parser callbacks. Use `withUsageOnError` for a usage banner on parse failure and `withHelp` for richer command-specific `-h` output.
