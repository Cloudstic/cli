...
- `cmd/cloudstic/` — CLI entry point (`package main`). `command.go` defines the recursive `command` tree (`leaf`/`group`) and its shared dispatcher; `commands.go` holds `commandRegistry()`, the ordered list that is the single source of truth for the command surface: `main.go`'s `runCmd()` dispatches from it, `printUsage()` renders its `COMMANDS` listing from it, and the shell-completion command lists are generated from it. Each command's `run*()` function lives in its own `cmd_*.go` file (e.g. `cmd_backup.go`, `cmd_key.go`). A `cmd_*.go` file holds the command surface — its args, flags, and `run*()`; workflows those commands call into live beside it under the area's own name, so `store_encryption.go` (choosing a store's encryption method and where its secrets live) and `store_health.go` (is a store reachable and initialized, including the AWS SSO re-auth path) sit next to `cmd_store.go` rather than inside it, since `profile new` and `setup` call into both. Subcommands: `init`, `backup`, `restore`, `list`, `ls`, `find`, `prune`, `forget`, `diff`, `copy`, `migrate`, `break-lock`, `key` (with `list`/`add-recovery`/`passwd`), `check`, `cat`, `profile`, `auth`, `store`, `source`, `setup`, `tui`, `completion`. Uses Go's `flag` package (no cobra/viper); `reorderArgs()` in `flags.go` allows flags after positional args. The interactive terminal UI lives in `cmd_tui*.go`.
  - The files that are **not** one command's surface, since this is where entropy collects and an unnamed file is where the next helper lands by default: `command.go`/`commands.go` (the tree and registry), `flags.go`/`flagspec.go`/`argspec.go` (global flag groups, the flag and positional descriptors), `usage.go`/`completion*.go` (both derived from the tree), `config.go` (flags → resolved `pkg/config` value), `runner.go`/`interactive.go` (I/O primitives and prompts), `storebuild.go`/`clientbuild.go`/`keychain.go` (construction from a resolved config), `format.go`/`json_output.go`/`copy_output.go` (presentation), `profile_render.go` (the profiles-file tables and their static validation), `profile_config.go` (ref-name rules, deterministic ordering), `profile_prompts.go`/`profile_secret.go`, `store_encryption.go`/`store_health.go`/`store_fields.go`, `profiling.go`, `crashinject*.go`. A new helper belongs in one of these or in a new file named for what it is — never in a `*_helpers.go`, which is what the two files deleted in this pass had become.
  - **The CLI consumes the public API, not the internals.** For anything the library exposes, `cmd/cloudstic` spells it the way an external caller must — `cloudstic.FileType`, `cloudstic.SourceInfo`, `cloudstic.WithRestorePath`, `cloudstic.ErrRepoLocked`, `cloudstic.BackupResult` — never `core.X` or `engine.X`. `TestCLIUsesOnlyPublicAPI` (`internal/apicheck`) enforces it over the package's imports, test files included, and lists in `cliPrivateInternals` the internal packages that are part of *this program* rather than of the library (`internal/ui`, `internal/tui`, `internal/app`, `internal/paths`, `internal/workstation`, `internal/logger`). The rest of `apicheck` proves the public surface is complete, alias-clean and buildable from outside; this is the one that proves it is *used*. If the CLI needs something that is genuinely missing, re-export it (see `repo.go`, `query.go`, `restore.go` for the alias pattern) rather than reaching past the boundary.
  - **Defaults that belong to construction are not flag defaults.** `-s3-region` binds to `""`, and `pkg/open` applies `us-east-1` at construction, so a configuration built from a profile and one built from flags cannot disagree. Pre-filling such a default on the flag put the value into every flag-built config and left `open`'s own default dead on that path — the two agreed only because the literal was copied into both packages. Same rule for anything else `pkg/open` decides; a flag whose help text states a default it does not itself apply should say where the default comes from (`-kms-region` says "from AWS environment", which is the AWS SDK's own resolution).
...
- `DiskCacheStore` (on by default) — Caches object bodies in a local directory, and is the reason the chain is drawn with a layer *below* `PackStore`. Built only when a caller passes `WithObjectCache(dir, budget)` — the library itself has no default, so a program embedding it gets no cache it did not ask for; `cmd/cloudstic` resolves `-object-cache-dir` / `-object-cache-bytes` (and their environment bindings, and `-no-object-cache`) into a `pkg/config.Client` that `pkg/open` applies, which is the same path every other client knob takes. The root package reads no environment variable of its own — a cache directory is a property of the machine, not of the repository, so it is also deliberately **not** a profile field. The CLI's default is `os.UserCacheDir()/cloudstic/objects`, resolved as a **late default** (`defaultObjectCachePath`, `config.go`) so the absolute path is never rendered into `-h` output or the golden files that pin it, and so resolving it creates nothing. `os.UserCacheDir` and not `paths.ConfigDir`: configuration is precious and may be backed up, this is disposable by construction, and on macOS the OS may purge it — right for a cache, wrong for a profiles file. **Test harnesses must isolate it**: `cleanEnv` (`e2e/harness_test.go`) and the testscript `Setup` (`cli_testscript_test.go`) both point it at a temp directory, because a suite that said nothing would write into the developer's own cache and could be served an object from a previous run's repository. **Its position is the design.** Directly above the backend, it sees every object in the form the store holds it — v2's `packs/` and v3's `blob/` alike — so one mechanism serves both formats with no knowledge of which wrote the repository; above `PackStore` it would see a v3 repository's objects but not a v2 repository's packs, which is what an earlier version got wrong. It caches every immutable namespace and names only the mutable exclusions (`index/`, `keys/`, `config`), so a namespace added to the format is cached rather than silently skipped. Entries carry a SHA-256 per 64 KiB block and are verified per read, over the blocks a read touches rather than the whole body — whole-body verification made a restore 2.2x slower, since a 4 KB range then cost a full read and hash of an 8 MB object. A block that fails is a miss, not an error. **The budget bounds the directory, not the process**: a save that would exceed it re-derives usage with a `readdir` and sweeps temp files older than an hour, because an in-memory counter cannot see another process's writes, a save killed between its write and its rename (one orphaned 8 MB temp file under a 1 MiB budget left 9.0x the budget on disk), or an entry the process forgot. It is a bound *checked at each save*, not a reservation held across processes: two writers that scan the same empty directory can both admit a large entry before either renames, so the directory can transiently exceed the budget by roughly one entry per concurrent writer, and is brought back by the next save that scans. An inter-process lock would close that and is deliberately not taken — a stale lock file would block a cache, which is a worse and longer-lived failure than a transient overshoot on data that is disposable by definition. `Client.Check` holds a bypass open for its run, through an unexported `readBypasser` interface rather than the concrete type: an entry served from disk is evidence about the cache, not about what the store holds now, so verifying through one would report a rotted repository healthy. Nothing here is repository format — the layer is removable without touching a byte on disk. See `docs/caching.md`.
...
| `CLOUDSTIC_OBJECT_CACHE_DIR` | Directory for the local object cache (`DiskCacheStore`); flag `-object-cache-dir`. Defaults to `os.UserCacheDir()/cloudstic/objects`; `-no-object-cache` disables the cache. |
| `CLOUDSTIC_OBJECT_CACHE_BYTES` | Bytes that directory may hold; flag `-object-cache-bytes`. Defaults to `storelayer.DiskCacheBudget` (2 GiB). Malformed is an error like any other env-bound flag; non-positive falls back to the default — no value of it removes the bound. |
