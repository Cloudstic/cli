...
func printUsage(out io.Writer) {
	...
	t.Flags([][2]string{
		...
		{"-b2-key-id <key>", ui.Env("Backblaze B2 application key ID", "B2_KEY_ID")},
		{"-b2-app-key <secret>", ui.Env("Backblaze B2 application key", "B2_APP_KEY")},
	...
	t.Flags([][2]string{
		...
		{"-b2-key-id <key>", "Backblaze B2 application key ID"},
		{"-b2-app-key <key>", "Backblaze B2 application key"},
		{"-b2-key-id-secret <ref>", "Secret reference for B2 key ID (env://, keychain://)"},
		{"-b2-app-key-secret <ref>", "Secret reference for B2 application key (env://, keychain://)"},
