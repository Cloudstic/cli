...
// EncryptedStore wraps an store.ObjectStore and transparently encrypts data on Put
...
func (s *EncryptedStore) Unwrap() store.ObjectStore { return s.ObjectStore }
...
func (s *EncryptedStore) Put(ctx context.Context, key string, data []byte) error {
	if strings.HasPrefix(key, store.KeySlotPrefix) {
