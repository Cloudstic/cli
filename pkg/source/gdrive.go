...
func saveToken(path string, token *oauth2.Token) error {
	...
	// We use the same atomic save logic as the file backend here for consistency.
	// But since this is pkg/source, we don't want to depend on internal/secretref's
	// private saveAtomic. For now, we'll do a simple safe write or just use os.WriteFile.
