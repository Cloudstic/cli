...
// Source implements Source for Google Drive. By default it backs up the
// entire "My Drive" root. Set DriveID in SourceConfig to back up a
...
func buildDriveService(ctx context.Context, cfg gDriveOptions) (*drive.Service, error) {
	...
	config := &oauth2.Config{
		ClientID:     sourceoauth.DefaultGoogleClientID,
		ClientSecret: sourceoauth.DefaultGoogleClientSecret,
...
func (s *Source) isSharedDrive() bool {
	return s.driveID != ""
}

func (s *Source) selectedRootID() string {
	if s.rootFolderID != "" {
...
func (s *Source) resolvePathToFolderID(ctx context.Context, path string) (string, error) {
...
func oauthClient(ctx context.Context, config *oauth2.Config, r *secretref.Resolver, tokRef, tokFile string) (*http.Client, error) {
	...
	pts := sourceoauth.NewPersistentTokenSource(ts, tok, func(t *oauth2.Token) error {
		if tokRef != "" {
...
func (s *Source) Size(ctx context.Context) (*source.SourceSize, error) {
	if !s.isSharedDrive() {
		...
		return &source.SourceSize{Bytes: about.StorageQuota.UsageInDrive}, nil
	}
...
func (s *Source) exportFile(fileID, exportMimeType string) (io.ReadCloser, error) {
	var resp *http.Response
	err := retry.Do(context.Background(), retry.DefaultPolicy(), func() error {
		var err error
