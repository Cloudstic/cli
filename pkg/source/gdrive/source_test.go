...
func TestToFileMeta_RegularFile(t *testing.T) {
	s := &Source{exclude: source.NewExcludeMatcher(nil)}
	f := &drive.File{
		Id:             "file1",
		Name:           "photo.jpg",
		MimeType:       "image/jpeg",
...
func TestVisitEntryWithPath_SkipNativeFiles(t *testing.T) {
	s := &Source{
		exclude:         source.NewExcludeMatcher(nil),
...
func TestChangeToFileChange_RecordsMimeType(t *testing.T) {
	...
	if fc.Type != source.ChangeUpsert {
		t.Errorf("Type = %v, want source.ChangeUpsert", fc.Type)
	}
	if s.mimeTypes["file1"] != "image/jpeg" {
		t.Errorf("mimeTypes[file1] = %q, want %q", s.mimeTypes["file1"], "image/jpeg")
	}
	...
	if fc2.Type != source.ChangeUpsert {
		t.Errorf("Type = %v, want source.ChangeUpsert", fc2.Type)
	}
	if s.mimeTypes["doc1"] != "application/vnd.google-apps.document" {
		t.Errorf("mimeTypes[doc1] = %q, want native MIME", s.mimeTypes["doc1"])
	}
...
func TestChangeToFileChange_DeletedFile(t *testing.T) {
	...
	if fc.Type != source.ChangeDelete {
		t.Errorf("Type = %v, want source.ChangeDelete", fc.Type)
	}
...
func TestGDriveInfo_MyDrive_Root(t *testing.T) {
	s := &Source{account: "user@gmail.com", rootPath: "/"}
	info := s.Info()
...
func TestGDriveInfo_MyDrive_Subfolder(t *testing.T) {
	s := &Source{account: "user@gmail.com", rootFolderID: "folder123", rootPath: "/myfolder"}
	info := s.Info()
...
func TestGDriveChangesInfo_Type(t *testing.T) {
	s := &ChangeSource{
		Source: Source{account: "user@gmail.com"},
	}
	info := s.Info()
