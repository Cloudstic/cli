...
func TestOneDrive_Options(t *testing.T) {
	...
	opts := []Option{
		...
		WithDriveName("Personal"),
		WithRootPath("/Documents"),
		WithTokenPath("/path/od.json"),
		WithTokenRef("keychain://od-tok"),
		WithExcludePatterns([]string{"Temp"}),
	}
...
func TestNew_RequiresResolverForTokenRef(t *testing.T) {
	_, err := New(context.Background(), WithTokenRef("config-token://onedrive/test"))
	if err == nil {
