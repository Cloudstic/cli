...
func TestOneDriveInfo(t *testing.T) {
	s := &Source{account: "user@outlook.com", rootPath: "/"}
	info := s.Info()
...
func TestOneDriveChangesInfo_Type(t *testing.T) {
	s := &ChangeSource{
		Source: Source{account: "user@outlook.com", rootPath: "/"},
	}
	info := s.Info()
...
func TestOneDriveFilterChangesByRootPath(t *testing.T) {
	s := &ChangeSource{
		Source: Source{rootPath: "/my/root/path"},
	}
	...
	if filtered[2].Type != source.ChangeDelete {
...
func TestOneDriveGetRootURL_EncodesPath(t *testing.T) {
	s := &Source{rootPath: "/Team Files/R?D"}
	got := s.getRootURL()
