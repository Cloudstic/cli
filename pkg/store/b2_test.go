...
// handful of calls B2Store actually makes is enough to exercise it without
// live B2 credentials or network access.

// fakeB2Object is the single stored version of an object under a given name;
// the fake server never versions objects, which is all B2Store needs.
...
// upload, download, list, and delete calls that B2Store's Put/Get/GetRange/
...
// fields B2Store's call paths actually populate or read.
...
// is what lets a single small fake object exercise B2Store's ranged-read and
...
func TestB2Store(t *testing.T) {
	...
	store, err := NewB2Store(bucketName, WithClient(client), WithPrefix("prefix/"))
	if err != nil {
		t.Fatalf("NewB2Store failed: %v", err)
	}
	...
	// store.ErrNotFound; blazer exposes no typed not-found error, so B2Store
	...
	// Flush is a documented no-op for B2Store.
	...
	// DeletePrefix
	if err := store.DeletePrefix(ctx, "test/"); err != nil {
