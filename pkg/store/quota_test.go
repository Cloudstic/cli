...
func TestQuotaStore_UnderBudget(t *testing.T) {
	...
	if err := qs.Put("chunk/aaa", make([]byte, 500)); err != nil {
		t.Fatalf("Put: %v", err)
	}
	if ctx.Err() != nil {
...
func TestQuotaStore_ExceedsBudget(t *testing.T) {
	...
	if err := qs.Put("chunk/aaa", make([]byte, 50)); err != nil {
	...
	if err := qs.Put("chunk/bbb", make([]byte, 60)); err != nil {
...
func TestQuotaStore_ExactBudget(t *testing.T) {
	...
	if err := qs.Put("chunk/aaa", make([]byte, 100)); err != nil {
		t.Fatalf("Put: %v", err)
	}
	if ctx.Err() != nil {
...
func TestQuotaStore_MultiplePutsAccumulate(t *testing.T) {
	...
	for i := range 10 {
		if err := qs.Put(fmt.Sprintf("chunk/%d", i), make([]byte, 10)); err != nil {
	...
	if err := qs.Put("chunk/overflow", make([]byte, 1)); err != nil {
